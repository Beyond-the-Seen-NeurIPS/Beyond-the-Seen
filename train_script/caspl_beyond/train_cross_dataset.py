...
GPU_NUM=0
SAVE_PATH="save_path/caspl_beyond_cross_dataset"
...
for (DATASET, DATA) in dataset_list:
    ...
    for (CFG_S1, KD_N_CTX_VISION_TEACHER, PROMPT_DEPTH_VISION_TEACHER) in CFG_S1_DICT:
        ...
        for (CFG_S2, N_CTX_VISION_STUDENT) in CFG_S2_LIST:
            OUT_PUT = ("caspl_beyond_s1_v" + str(KD_N_CTX_VISION_TEACHER) +
            ...
            for COUNTER in range(1, 2):
                cmd = f"CUDA_VISIBLE_DEVICES={GPU_NUM} bash scripts/caspl/beyond/xd_train_student.sh \
                ...
                for (DATASET_TEST, DATA_TEST) in dataset_list_test:
                    cmd = f"CUDA_VISIBLE_DEVICES={GPU_NUM} bash scripts/caspl/beyond/xd_test_student.sh \
